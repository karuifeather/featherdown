...
/**
 * Remark plugin: strips ` {#custom-id}` from heading text and sets `data.id` /
 * `hProperties.id` so remark-rehype carries the id into HTML. `rehype-slug`
 * skips headings that already have an `id` on the hast node.
 */
